...
from datasets.open_world_clad import BASE_CLAD_CLASS_NAMES
    ...
    def accumulate(self):
        for class_label_ind, class_label in enumerate(self.voc_gt.CLASS_NAMES):
            ...
            self.rec, self.prec, self.AP[class_label_ind, ovthresh_ind], self.unk_det_as_known, \
                ...
                self.voc_gt.img_anns, self.voc_gt.ids, class_label, ovthresh=ovthresh / 100.0, use_07_metric=self.use_07_metric, known_classes=self.known_classes) #[-1]
...
def parse_rec(img_annot, known_classes):
    """ Parse an image annotation following PASCAL VOC format """
    
    objects = []
    for obj in img_annot:
        obj_struct = {}
        cls_name = BASE_CLAD_CLASS_NAMES[obj['category_id']]
        if cls_name not in known_classes:
            cls_name = 'unknown'
        obj_struct['name'] = cls_name
        obj_struct['difficult'] = int(any(obj[key] == 1 for key in ['truncated', 'occluded', 'iscrowd']))
...
def voc_eval(detpath,
    ...
    if isinstance(imagesetfile, list):
        imagenames = [str(x) for x in imagesetfile]
    else:
    ...
    for i, imagename in enumerate(imagenames):
        recs[imagename] = parse_rec(annopath[int(imagename)], tuple(known_classes))
