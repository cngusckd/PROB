...
class OWCladDetection(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        self.img_folder = os.path.join(root, 'CLAD_PROB_FORMAT', 'data', 'OWOD', split)
    ...
    def extract_clad_fns(self, root, image_set):
        splits_dir = os.path.join(root, 'CLAD_PROB_FORMAT', 'data', 'OWOD')
...
# This is the original code from the repository (https://github.com/VerwimpEli/CLAD)
...
def create_val_from_trainset(trainset: CladDetection, root, val_transform, split, proportion=0.1):
    ...
    return CladDetection(root, val_ids, os.path.join(root, 'CLAD_PROB_FORMAT', 'data', 'OWOD', 'annotations',
