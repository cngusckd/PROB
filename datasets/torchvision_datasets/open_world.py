...
class swOWDetection(VisionDataset):
    """`OWOD in Pascal VOC format <http://host.robots.ox.ac.uk/pascal/VOC/>`_ Detection Dataset.
    ...
    def __init__(self,
                 ...
                 filter_pct=-1,
                 dataset='OWDETR'):
        super(swOWDetection, self).__init__(transforms)
        self.images = []
        self.annotations = []
        self.imgids = []
        self.imgid2annotations = {}
        ...
        self.txt_root=str(txt_root)
        self.data_root=str(data_root)
        annotation_dir = os.path.join(self.data_root, 'Annotations')
        image_dir = os.path.join(self.data_root, 'JPEGImages')

        file_names = self.extract_fns(image_set, self.txt_root)
        if image_set == 'voc2007_trainval':
            print('PASCAL-VOC2007 dataset used; clearing images with missing object classes')
            prev_intro_cls = self.args.PREV_INTRODUCED_CLS
            curr_intro_cls = self.args.CUR_INTRODUCED_CLS
