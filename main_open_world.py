...
def main(args):
   ...
   # 1. --freeze_mode 인자에 따라 파라미터를 조건부로 동결합니다.
    ...
    # === Base dataset configuration (for evaluation) ===
    if args.eval:
        # EVAL mode: Use validation dataset for base_ds
        ...
        # TRAIN mode: Base dataset can be None or minimal (not used for training)
        base_ds = None
    ...
    if args.pretrain:
        ...
        # === EVAL Mode: Run evaluation only and return ===
        if args.eval:
            print('=== EVAL Mode: Running evaluation only ===')
    ...
    if args.resume:
        ...
        if args.eval:
            print('=== EVAL Mode: Running evaluation with resumed model ===')
            test_stats, coco_evaluator = evaluate(model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args)
            if args.output_dir:
