...
python -u main_open_world.py \
    ...
    --wandb_project ${PORJECT_NAME} --wandb_name "${WANDB_NAME}_t1" --exemplar_replay_selection --exemplar_replay_max_length 1000\
...
python -u main_open_world.py \
    ...
    --wandb_project ${PORJECT_NAME} --wandb_name "${WANDB_NAME}_t2" --batch_size ${BATCH_SIZE}\
...
python -u main_open_world.py \
    ...
    --wandb_project ${PORJECT_NAME} --wandb_name "${WANDB_NAME}_t2_ft"\
    --model_type 'prob' --batch_size ${BATCH_SIZE} --obj_loss_coef 8e-4 --obj_temp 1.3\
    --pretrain "${EXP_DIR}/t2/checkpoint00$(( EPOCHS + 19 )).pth"\
    ${PY_ARGS}
