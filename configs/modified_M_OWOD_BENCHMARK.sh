...
EXP_DIR=exps/MOWODB/CLAD
PY_ARGS=${@:1}
WANDB_NAME=CLAD_CL_FIX
...
python -u main_open_world.py \
    ...
    --model_type 'prob' --obj_loss_coef 8e-4 --obj_temp 1.3 --batch_size ${BATCH_SIZE}\
    --wandb_project "PROB_CHU" --wandb_name "${WANDB_NAME}_t1" --exemplar_replay_selection --exemplar_replay_max_length 1000\
...
python -u main_open_world.py \
    ...
    --train_set 'clad_t2_train_2025ver' --test_set 'clad_test' --epochs ${EPOCHS}\
    --model_type 'prob' --obj_loss_coef 8e-4 --obj_temp 1.3 --freeze_prob_model \
    --wandb_project "PROB_CHU" --wandb_name "${WANDB_NAME}_t2" --batch_size ${BATCH_SIZE}\
    ...
    --pretrain "${EXP_DIR}/t1/checkpoint00$(( EPOCHS - 1 )).pth" --lr 2e-5\
    ${PY_ARGS}
...
python -u main_open_world.py \
    ...
    --train_set "${WANDB_NAME}/${REPLAY_NAME}_t2_ft.txt" --test_set 'clad_test' --epochs ${EPOCHS} --lr_drop 7\
    --model_type 'prob' --batch_size ${BATCH_SIZE} --obj_loss_coef 8e-4 --obj_temp 1.3\
    --wandb_name "${WANDB_NAME}_t2_ft"\
    --pretrain "${EXP_DIR}/t2/checkpoint00$(( EPOCHS - 1 )).pth"\
    ${PY_ARGS}
...
python -u main_open_world.py \
    ...
    --train_set 'clad_t3_train_2025ver' --test_set 'clad_test' --epochs ${EPOCHS}\
    --model_type 'prob' --obj_loss_coef 8e-4 --obj_temp 1.3 --freeze_prob_model \
    --wandb_project "PROB_CHU" --wandb_name "${WANDB_NAME}_t2" --batch_size ${BATCH_SIZE}\
    --exemplar_replay_selection --exemplar_replay_max_length 1000 --exemplar_replay_dir ${WANDB_NAME}\
    --exemplar_replay_prev_file "${REPLAY_NAME}_t1_ft.txt" --exemplar_replay_cur_file "${REPLAY_NAME}_t2_ft.txt"\
...
python -u main_open_world.py \
    ...
    --train_set "${WANDB_NAME}/${REPLAY_NAME}_t3_ft.txt" --test_set 'clad_test' --epochs ${EPOCHS} --lr_drop 7\
    --model_type 'prob' --batch_size ${BATCH_SIZE} --obj_loss_coef 8e-4 --obj_temp 1.3\
    --wandb_name "${WANDB_NAME}_t2_ft"\
    --pretrain "${EXP_DIR}/t2/checkpoint00$(( EPOCHS - 1 )).pth"\
    ${PY_ARGS}