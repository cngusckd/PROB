...
python -m memory_profiler main_open_world.py \
    ...
    --exemplar_replay_dir ${WANDB_NAME} --exemplar_replay_cur_file "${REPLAY_NAME}_t1_ft.txt" --transformer_weights './checkpoints/PROB_transformer.pth'\
    ${PY_ARGS}
...
python -u main_open_world.py \
    ...
    --wandb_project ${PROJECT_NAME} --wandb_name "${WANDB_NAME}_t2" --batch_size ${BATCH_SIZE} --freeze_mode ${FREEZE_MODEL}\
...
python -u main_open_world.py \
    ...
    --wandb_project ${PROJECT_NAME} --wandb_name "${WANDB_NAME}_t2_ft" --freeze_mode ${FREEZE_MODEL}\
...
python -u main_open_world.py \
    ...
    --model_type 'prob' --batch_size ${BATCH_SIZE} --obj_loss_coef 8e-4 --obj_temp 1.3 --freeze_mode ${FREEZE_MODEL}\
